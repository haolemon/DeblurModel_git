...
class Mixerlayer(nn.Module):
    def __init__(self, dim, mix_kernel_size, bias):
        ...
        self.dense_depth_1 = nn.Conv2d(dim, dim, kernel_size=mix_kernel_size, stride=1, padding=mix_kernel_size // 2,
                                       groups=dim, bias=bias)

        self.dense_point_1 = nn.Conv2d(dim, dim, 1)

        self.dense_depth_2 = nn.Conv2d(dim, dim, kernel_size=mix_kernel_size, stride=1, padding=mix_kernel_size // 2,
                                       groups=dim, bias=bias)
...
class LaKDNet(nn.Module):
    def __init__(self,
        ...
        self.encoder_level1 = nn.Sequential(*[
            Mixerblock(dim=dim, mix_kernel_size=mix_kernel_size[0], ffn_expansion_factor=ffn_expansion_factor,
                       bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])

        self.down1_2 = Downsample(dim)
        self.encoder_level2 = nn.Sequential(*[Mixerblock(dim=int(dim * 2 ** 1), mix_kernel_size=mix_kernel_size[1],
                                                         ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                                                         LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])

        self.down2_3 = Downsample(int(dim * 2 ** 1))
        self.encoder_level3 = nn.Sequential(*[Mixerblock(dim=int(dim * 2 ** 2), mix_kernel_size=mix_kernel_size[2],
                                                         ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                                                         LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])])

        self.down3_4 = Downsample(int(dim * 2 ** 2))
        self.latent = nn.Sequential(*[Mixerblock(dim=int(dim * 2 ** 3), mix_kernel_size=mix_kernel_size[3],
                                                 ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                                                 LayerNorm_type=LayerNorm_type) for i in range(num_blocks[3])])

        self.up4_3 = Upsample(int(dim * 2 ** 3))
        self.reduce_chan_level3 = nn.Conv2d(int(dim * 2 ** 3), int(dim * 2 ** 2), kernel_size=1, bias=bias)
        self.decoder_level3 = nn.Sequential(*[Mixerblock(dim=int(dim * 2 ** 2), mix_kernel_size=mix_kernel_size[2],
                                                         ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                                                         LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])])

        self.up3_2 = Upsample(int(dim * 2 ** 2))
        self.reduce_chan_level2 = nn.Conv2d(int(dim * 2 ** 2), int(dim * 2 ** 1), kernel_size=1, bias=bias)
        self.decoder_level2 = nn.Sequential(*[Mixerblock(dim=int(dim * 2 ** 1), mix_kernel_size=mix_kernel_size[1],
                                                         ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                                                         LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])

        self.up2_1 = Upsample(int(dim * 2 ** 1))
        self.decoder_level1 = nn.Sequential(*[Mixerblock(dim=int(dim * 2 ** 1), mix_kernel_size=mix_kernel_size[0],
                                                         ffn_expansion_factor=ffn_expansion_factor, bias=bias,
...
if __name__ == '__main__':
    ...
    model = LaKDNet(inp_channels=inp_channels, out_channels=out_channels, dim=dim, num_blocks=num_blocks,
